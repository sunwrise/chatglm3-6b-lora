...
from transformers import TrainerCallback #为了实现早停逻辑，新添加的代码
...
class DataManager(object):
    ...
    def get_dataset(
        ...
        print(f"原始列名：{orig_dataset.column_names}")  # 打印列名
...
class EarlyStoppingCallback(TrainerCallback):  
    def __init__(self, threshold: float = 3.3):  
        self.threshold = threshold

    def on_step_end(self, args, state, control, **kwargs):
        # 获取最后一个step的loss
        # print("\n进入到回传函数，查看loss是否低于阈值")
        train_loss = state.log_history[-1].get("loss")
        if train_loss is not None and train_loss < self.threshold:
            print(f"\n\nTraining stopped early as loss {train_loss} is below threshold {self.threshold}.")
...
def main(
    ...
    # 创建自定义回调   
    early_stopping_callback = EarlyStoppingCallback(threshold=3.25) 
    ...
    trainer = Seq2SeqTrainer(
        ...
        callbacks=[early_stopping_callback],  #补充自定义回调函数
    )
